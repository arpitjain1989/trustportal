...
class VendorRegister extends Component
{
    ...
    public function saveStep1()
    {
        ...
        {
            session()->flash('vendor_message', 'This number is already registered');
        ...
        else
        {
            ...
            $this->step++;
            $this->is_disabled = false;
            session()->flash('vendor_message', 'OTP sent successfully !');
        }
    }
    ...
    public function saveStep2()
    {
        ...
        if($response == 1)
        {
            try
            {
                $user_id = DB::table('vendors')->insertGetId([
                            'name'=> $this->name,
                            'email'=> $this->email,
                            'mobile'=> $this->mobile,
                ...
                Auth::guard('vendor')->loginUsingId($user_id);
                $this->step++;
                $this->emitTo('header', '$refresh');
            }
            catch(\Exception $e)
            {
                Log::info(" Vendor registration error.".$e);
                session()->flash('vendor_message', 'Mobile / Email already exists');
            }
        }
    }
